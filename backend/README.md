...
## AI 비전 기반 착장 분석기
- 현재 업로드 분석은 AI 비전 결과를 우선 사용하고, 규칙 기반 분석은 AI provider가 비활성화되었거나 응답이 비어 있을 때만 fallback 으로 사용합니다.
...
- 2026-05-11 `minicpm-v:latest` 실검증에서는 `화이트 셔츠 / 블루 카디건`만 반환해 `bottom/accessory`를 놓쳤고, 현재 샘플 기준 기본 모델 승격은 보류했습니다.
...
- 2026-04-29 이후 업로드 런타임은 AI 결과가 존재하면 해당 결과만 유지하고, 규칙 기반 결과는 AI가 비어 있을 때만 fallback 으로 사용하도록 전환했습니다.
...
- 2026-05-03 accessory 후처리를 보강해 `메탈 안경`, `진주 귀걸이`, `링 귀걸이` descriptor를 query에 더 잘 남기고, `칼라` 같은 unsupported accessory 라벨과 단일 추가 jewelry 보정 오탐은 더 보수적으로 제거했습니다.
