...
- 비전 분석 결과가 하나라도 존재하면 해당 AI 결과를 최종 detected_items의 기준으로 사용하고, 규칙 기반 결과를 자동 병합하지 않는다.
- 선택적 Gemini 보정이 켜져 있으면 다른 AI provider의 결과를 교체/추가하는 용도로만 사용하고, 규칙 기반 결과는 보정 대상 후보를 고르는 힌트 수준으로만 제한한다.
